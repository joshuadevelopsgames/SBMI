...
2. **Environment variables**: `SESSION_SECRET` and `NEXT_PUBLIC_APP_URL` are set. Add **`DATABASE_URL`** (see step 2). Optional: `MONTHLY_CONTRIBUTION_CENTS`, `PENALTY_AMOUNT_CENTS`, `RESEND_API_KEY`, `EMAIL_FROM`, `DEMO_MODE`.
3. Deploy from CLI: `npm run deploy` or `vercel --prod` from `sbmi.ca/`.

### Git deployment (push → auto deploy)

For pushes to GitHub to trigger a Vercel deploy, set these in **Vercel Dashboard → Project sbmi.ca → Settings**:

| Setting | Where | Value |
|--------|--------|--------|
| **Root Directory** | General → Root Directory | `sbmi.ca` (required: app lives in this subfolder) |
| **Production Branch** | Git → Production Branch | `main` |
| **Repo** | Git | Your GitHub repo (e.g. `joshuadevelopsgames/SBMI`) |

Save. After that, every push to `main` will deploy. Without Root Directory = `sbmi.ca`, Git builds from repo root and will fail or deploy the wrong thing.
