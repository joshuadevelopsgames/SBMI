...
/** SOW US14: reset link does not expire for community usability — use far-future expiry. */
const PASSWORD_RESET_EXPIRY_YEARS = 100;
...
export async function createPasswordResetToken(userId: string): Promise<string> {
  const token = generateToken();
  const expiresAt = new Date();
  expiresAt.setFullYear(expiresAt.getFullYear() + PASSWORD_RESET_EXPIRY_YEARS);
