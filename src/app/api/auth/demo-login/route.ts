...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const users = await prisma.$queryRaw<
      Array<{ id: string; roleName: string; memberId: string | null }>
    >(
      Prisma.sql`
        SELECT u.id, r.name AS "roleName", u."memberId"
        FROM "User" u
    ...
    const redirectTo = await getMemberDashboardRedirect(user.roleName, user.memberId);
